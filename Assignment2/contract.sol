...
contract Creator {
  ...
  // Limited to 3 media per creator
  // Returns (size, media_id, cost)
  function get_all_media(bool is_individual) view public returns (uint, uint[3], uint[3]) {
    uint[3] memory media_ids;
    uint[3] memory costs;
...
contract MainContract {
  ...
  // Get all media for consumer
  function get_all_media(bool is_individual) view public returns (address[], uint[], uint[3][], uint[3][]) {
    uint[] memory sizes = new uint[](creator_addr.length);
    uint[3][] memory media_id_array = new uint[3][](creator_addr.length);
    uint[3][] memory cost_array = new uint[3][](creator_addr.length);

    for(uint i=0; i<creator_addr.length; ++i){
      uint size;
      uint[3] memory media_id;
      uint[3] memory cost;
